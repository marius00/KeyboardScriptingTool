...
namespace KST.UI {
    public partial class AddModifyEntry : Form {
        ...
        /// <summary>
        /// Opens the script wizard, re-opening the current script's configuration if it was generated by the wizard.
        /// </summary>
        private void btnWizard_Click(object sender, EventArgs e) {
            var dialog = new ScriptWizardForm();
            var current = Path.Combine(AppPaths.SettingsFolder, tbScript.Text);

            if (tbScript.Text.Length > 0 && File.Exists(current)) {
                var config = ScriptGenerator.TryParseConfig(File.ReadAllText(current));
                if (config != null) {
                    dialog.Config = config;
                }
                else if (MessageBox.Show(
                             $"\"{tbScript.Text}\" was not created by the wizard, so its configuration cannot be read back.\r\n\r\nStart a new script instead?",
                             "Not a wizard script",
            ...
            if (dialog.Config.Bindings.Count == 0 && string.IsNullOrEmpty(dialog.Config.ScriptName)) {
                // Name the script after whatever the user has already filled in.
                dialog.Config.ScriptName = tbProcess.Text.Trim();
            }

            if (dialog.ShowDialog(this) == DialogResult.OK) {
                tbScript.Text = dialog.ScriptFileName;
            }
        }
